...
5. Compile the protobuf to a Python module. The example below emits
   ...
   Quick sanity check: in the same shell where you'll run the app, make sure the
   generated module is importable and contains the expected class:

   ```bash
   python - <<'PY'
   import importlib

   mod = importlib.import_module("$ZEROBUS_PROTO_MODULE")
   print("Loaded", mod)
   print("Has message?", hasattr(mod, "$ZEROBUS_PROTO_MESSAGE"))
   PY
   ```
...
- `Zerobus ingest SDK is not configured: ...` – The SDK could not initialize a
  stream. Check that:
  - The `databricks-zerobus-ingest-sdk` dependency is installed (it ships in
    `requirements.txt`).
  - `ZEROBUS_SERVER_ENDPOINT`, `DATABRICKS_HOST`, `ZEROBUS_CLIENT_ID`,
    `ZEROBUS_CLIENT_SECRET`, `ZEROBUS_PROTO_MODULE`, and `ZEROBUS_PROTO_MESSAGE`
    are set (run `set -a; source .env; set +a` in the same shell before
    launching the app).
  - The generated `*_pb2.py` file is on `PYTHONPATH` (placing it in the repo
    root as shown above keeps it importable).
  - `ZEROBUS_PROTO_MESSAGE` matches the class name inside the generated module
    (verify with the quick sanity check snippet above).
